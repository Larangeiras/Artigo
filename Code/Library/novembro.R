...
############################################################################
Histograma_log_DP_1.sel <- function(dados,x.min,x.max,y.max)
{
  dados_para_saida <- log_DP_1_varias_matrizes(dados)
  Histograma_log_DP_1 <- hist(dados_para_saida, breaks = "FD", probability = T, 
...
############################################################################
Plot_Dispersao_s1_s2.sel <- function(dados,x.min,x.max,y.min,y.max)
{
  y <- log_DP_1_varias_matrizes(dados)
  x <- log_DP_2_varias_matrizes(dados)
  Plot_Dispersao_s1_s2 <- plot( x, y, xlab = expression(log~sigma[2]),
                                ylab = expression(log~sigma[1]), pch = 20,
...
############################################################################
Histograma_Variancia_Efetiva.sel <- function(dados,x.min,x.max,y.max)
{
  dados_para_saida <- Variancia_efetiva_de_varias_matrizes(dados)
  Histograma_Variancia_Efetiva <- hist(dados_para_saida, breaks = "FD", probability = T,
                                       xlim = c(x.min,x.max), ylim = c(0,y.max), 
...
############################################################################
Histograma_Dependencia_Efetiva.sel <- function(dados,x.min,x.max,y.max)
{
  dados_para_saida <- Dependencia_efetiva_de_varias_matrizes(dados)
  Histograma_Dependencia_Efetiva <- hist(dados_para_saida, breaks = "FD", probability = T,
                                         xlim = c(x.min,x.max), ylim = c(0,y.max),
...
############################################################################
Plot_Dispersao_s1_r_12.sel <- function(dados,y.min,y.max)
{
  y <- log_DP_1_varias_matrizes(dados)
  x <- Corr_marginal_12_varias_matrizes(dados)
  Plot_Dispersao_s1_r12 <- plot( x, y, xlab = expression(rho[12]),
                                 ylab = expression(log~sigma[1]),
                                 pch = 20,
                                 # lab = c(2,2,4),
                                 xlim = c(-1,1), ylim = c(y.min,y.max)
                                 )
}
############################################################################
...
############################################################################
Plot_Dispersao_s1_r_23.sel <- function(dados,y.min,y.max)
{
  y <- log_DP_1_varias_matrizes(dados)
  x <- Corr_marginal_23_varias_matrizes(dados)
  Plot_Dispersao_s1_r23 <- plot( x, y, xlab = expression(rho[23]),
                                 ylab = expression(log~sigma[1]),
                                 pch = 20,
                                 # lab = c(2,2,4),
                                 xlim = c(-1,1),
...
############################################################################
Histograma_Corr_marginal_12.sel <- function(dados,y.max)
{
  dados_para_saida <- Corr_marginal_12_varias_matrizes(dados)
  Histograma_Corr_marginal_12 <- hist(dados_para_saida, breaks ="FD", probability = T,
                                      xlab = expression(rho[12]) , main = "", xlim=c(-1,1),
                                      ylim = c(0,y.max))
  lines(density(dados_para_saida), col="green")
}
############################################################################
...
############################################################################
# Matrizes_Cov_Complex.sel = function( G  , linha_inicial , linha_final ,  coluna_inicial , coluna_final )
# {
#   
#   if(    (linha_inicial>=1 && linha_inicial<=dim([[G]][[1]])[1] )  &&  ( linha_final>=1 && linha_final<=dim([[G]][[1]])[1] ) && 
#          ( coluna_inicial>=1 && coluna_inicial<=dim([[G]][[1]])[2] ) && ( coluna_final>=1 && coluna_final<=dim([[G]][[1]])[2] )  )
#   {
#     
#     resultados = list(  ) 
#     contador = 1
...
# VisCovMatComplex.selecionada <- function(entrada, I1, I2, I3 )
# {
#   I1 <- Matrizes_Cov_Complex(I1[[1]],I1[[2]],I1[[3]],I1[[4]],I1[[5]])
#   I2 <- Matrizes_Cov_Complex(I2[[1]],I2[[2]],I2[[3]],I2[[4]],I2[[5]])
#   I3 <- Matrizes_Cov_Complex(I3[[1]],I3[[2]],I3[[3]],I3[[4]],I3[[5]])
#   
...
#     {
#       x.min = min(dados1[[3]],dados2[[3]],dados3[[3]])
#       x.max = max(dados1[[3]],dados2[[3]],dados3[[3]])
#       y.max = max(hist(dados1[[3]],plot = F)$density,
#                   hist(dados2[[3]],plot = F)$density,
#                   hist(dados3[[3]],plot = F)$density)
#       
#       graficos = list(Histograma_log_DP_1.sel(I1,x.min,x.max,y.max),
#                       Histograma_log_DP_1.sel(I2,x.min,x.max,y.max),
#                       Histograma_log_DP_1.sel(I3,x.min,x.max,y.max))
#     }
#     if(entrada == graficos[2])
#     {
#       x.min = min(dados1[[1]],dados2[[1]],dados3[[1]])
#       x.max = max(dados1[[1]],dados2[[1]],dados3[[1]])
#       y.min = min(dados1[[7]],dados2[[7]],dados3[[7]])
#       y.max = max(dados1[[7]],dados2[[7]],dados3[[7]])
#       graficos = list(Plot_Dispersao_s1_s2.sel(I1,x.min,x.max,y.min,y.max),
#                       Plot_Dispersao_s1_s2.sel(I2,x.min,x.max,y.min,y.max),
#                       Plot_Dispersao_s1_s2.sel(I3,x.min,x.max,y.min,y.max)
#                       )
#     }
...
#       x.min = min(dados1[[1]],dados2[[1]],dados3[[1]])
#       x.max = max(dados1[[1]],dados2[[1]],dados3[[1]])
#       y.max = max(hist(dados1[[1]],plot = F)$density,
#                   hist(dados2[[1]],plot = F)$density,
#                   hist(dados3[[1]],plot = F)$density)
#       graficos = list(Histograma_Variancia_Efetiva.sel(I1,x.min,x.max,y.max),
#                       Histograma_Variancia_Efetiva.sel(I2,x.min,x.max,y.max),
#                       Histograma_Variancia_Efetiva.sel(I3,x.min,x.max,y.max)
#                       )
#     }
#     if(entrada == graficos[4])
#     {
#       x.min = min(dados1[[2]],dados2[[2]],dados3[[2]])
#       x.max = max(dados1[[2]],dados2[[2]],dados3[[2]])
#       y.max = max(hist(dados1[[2]],plot = F)$density,
#                   hist(dados2[[2]],plot = F)$density,
#                   hist(dados3[[2]],plot = F)$density)
#       graficos = list(Histograma_Dependencia_Efetiva.sel(I1,x.min,x.max,y.max),
#                       Histograma_Dependencia_Efetiva.sel(I2,x.min,x.max,y.max),
#                       Histograma_Dependencia_Efetiva.sel(I3,x.min,x.max,y.max)
#                       )
#     }
#     if(entrada == graficos[5])
#     {
#       y.max = max(hist(dados1[[4]],plot = F)$density,
#                   hist(dados2[[4]],plot = F)$density,
#                   hist(dados3[[4]],plot = F)$density)
#       graficos = list(Histograma_Corr_marginal_12.sel(I1,y.max),
#                       Histograma_Corr_marginal_12.sel(I2,y.max),
#                       Histograma_Corr_marginal_12.sel(I3,y.max)
#                       )
#     }
#     if(entrada == graficos[6])
#     {
#       graficos = list(Plot_Dispersao_r12_r_23(I1), Plot_Dispersao_r12_r_23(I2), Plot_Dispersao_r12_r_23(I3))
#     }
#     if(entrada == graficos[7])
#     {
#       y.min = min(dados1[[3]],dados2[[3]],dados3[[3]])
#       y.max = max(dados1[[4]],dados2[[4]],dados3[[4]])
#       graficos = list(Plot_Dispersao_s1_r_12.sel(I1,y.min,y.max),
#                       Plot_Dispersao_s1_r_12.sel(I2,y.min,y.max),
#                       Plot_Dispersao_s1_r_12.sel(I3,y.min,y.max)
#                       )
#     }
#     if(entrada == graficos[8])
#     {
#       y.min = min(dados1[[3]],dados2[[3]],dados3[[3]])
#       y.max = max(dados1[[8]],dados2[[8]],dados3[[8]])
#       graficos = list(Plot_Dispersao_s1_r_23.sel(I1,y.min,y.max),
#                       Plot_Dispersao_s1_r_23.sel(I2,y.min,y.max),
#                       Plot_Dispersao_s1_r_23.sel(I3,y.min,y.max)
#                       )
#     }
#     if(entrada == graficos[9])
#     {
#       graficos = list(CORRELACAO_ESPACIAL(I1), CORRELACAO_ESPACIAL(I2), CORRELACAO_ESPACIAL(I3))
#     }
#   
# }
...
VisCovMatComplex.selecionada <- function(entrada, lista.imagens )
{
  ...
  { # Para 2 imagens
    ...
    if(entrada == graficos[1])
    {
      x.min = min(dados1[[3]],dados2[[3]])
      x.max = max(dados1[[3]],dados2[[3]])
      y.max = max(hist(dados1[[3]],plot = F)$density,
                  hist(dados2[[3]],plot = F)$density
      )
    ...
    if(entrada == graficos[2])
    {
      x.min = min(dados1[[1]],dados2[[1]])
      x.max = max(dados1[[1]],dados2[[1]])
      y.min = min(dados1[[7]],dados2[[7]])
      y.max = max(dados1[[7]],dados2[[7]])
      graficos = list(Plot_Dispersao_s1_s2.sel(I1,x.min,x.max,y.min,y.max),
                      Plot_Dispersao_s1_s2.sel(I2,x.min,x.max,y.min,y.max)
      )
    }
    ...
    if(entrada == graficos[4])
    {
      x.min = min(dados1[[2]],dados2[[2]])
      x.max = max(dados1[[2]],dados2[[2]])
      y.max = max(hist(dados1[[2]],plot = F)$density,
                  hist(dados2[[2]],plot = F)$density
      )
      graficos = list(Histograma_Dependencia_Efetiva.sel(I1,x.min,x.max,y.max),
    ...
    if(entrada == graficos[6])
    {
      graficos = list(Plot_Dispersao_r12_r_23(I1), Plot_Dispersao_r12_r_23(I2))
    }
    if(entrada == graficos[7])
    {
      y.min = min(dados1[[3]],dados2[[3]])
      y.max = max(dados1[[4]],dados2[[4]])
      graficos = list(Plot_Dispersao_s1_r_12.sel(I1,y.min,y.max),
                      Plot_Dispersao_s1_r_12.sel(I2,y.min,y.max)
      )
    }
    ...
    if(entrada == graficos[9])
    {
      graficos = list(CORRELACAO_ESPACIAL(I1), CORRELACAO_ESPACIAL(I2))
    }
  }
  ...
  { # Para 4 imagens
    ...
    if(entrada == graficos[1])
    {
      x.min = min(dados1[[3]],dados2[[3]],dados3[[3]],dados4[[3]])
      x.max = max(dados1[[3]],dados2[[3]],dados3[[3]],dados4[[3]])
      y.max = max(hist(dados1[[3]],plot = F)$density,
                  hist(dados2[[3]],plot = F)$density,
                  hist(dados3[[3]],plot = F)$density,
                  hist(dados4[[3]],plot = F)$density
      )

      graficos = list(Histograma_log_DP_1.sel(I1,x.min,x.max,y.max),
                      Histograma_log_DP_1.sel(I2,x.min,x.max,y.max),
                      Histograma_log_DP_1.sel(I3,x.min,x.max,y.max),
                      Histograma_log_DP_1.sel(I4,x.min,x.max,y.max)
      )
    }
    if(entrada == graficos[2])
    {
      x.min = min(dados1[[1]],dados2[[1]],dados3[[1]],dados4[[1]])
      x.max = max(dados1[[1]],dados2[[1]],dados3[[1]],dados4[[1]])
      y.min = min(dados1[[7]],dados2[[7]],dados3[[7]],dados4[[7]])
      y.max = max(dados1[[7]],dados2[[7]],dados3[[7]],dados4[[7]])
      graficos = list(Plot_Dispersao_s1_s2.sel(I1,x.min,x.max,y.min,y.max),
                      Plot_Dispersao_s1_s2.sel(I2,x.min,x.max,y.min,y.max),
                      Plot_Dispersao_s1_s2.sel(I3,x.min,x.max,y.min,y.max),
                      Plot_Dispersao_s1_s2.sel(I4,x.min,x.max,y.min,y.max)
      )
    }
    if(entrada == graficos[3])
    {
      ...
      y.max = max(hist(dados1[[1]],plot = F)$density,
                  hist(dados2[[1]],plot = F)$density,
                  hist(dados3[[1]],plot = F)$density,
                  hist(dados4[[1]],plot = F)$density)
      graficos = list(Histograma_Variancia_Efetiva.sel(I1,x.min,x.max,y.max),
                      Histograma_Variancia_Efetiva.sel(I2,x.min,x.max,y.max),
                      Histograma_Variancia_Efetiva.sel(I3,x.min,x.max,y.max),
                      Histograma_Variancia_Efetiva.sel(I4,x.min,x.max,y.max)
      )
    }
    if(entrada == graficos[4])
    {
      x.min = min(dados1[[2]],dados2[[2]],dados3[[2]],dados4[[2]])
      x.max = max(dados1[[2]],dados2[[2]],dados3[[2]],dados4[[2]])
      y.max = max(hist(dados1[[2]],plot = F)$density,
                  hist(dados2[[2]],plot = F)$density,
                  hist(dados3[[2]],plot = F)$density,
                  hist(dados4[[2]],plot = F)$density
      )
      graficos = list(Histograma_Dependencia_Efetiva.sel(I1,x.min,x.max,y.max),
                      Histograma_Dependencia_Efetiva.sel(I2,x.min,x.max,y.max),
                      Histograma_Dependencia_Efetiva.sel(I3,x.min,x.max,y.max),
                      Histograma_Dependencia_Efetiva.sel(I4,x.min,x.max,y.max)
      )
    }
    if(entrada == graficos[5])
    {
      y.max = max(hist(dados1[[4]],plot = F)$density,
                  hist(dados2[[4]],plot = F)$density,
                  hist(dados3[[4]],plot = F)$density,
                  hist(dados4[[4]],plot = F)$density
      )
      graficos = list(Histograma_Corr_marginal_12.sel(I1,y.max),
                      Histograma_Corr_marginal_12.sel(I2,y.max),
                      Histograma_Corr_marginal_12.sel(I3,y.max),
                      Histograma_Corr_marginal_12.sel(I4,y.max)
      )
    }
    if(entrada == graficos[6])
    {
      graficos = list(Plot_Dispersao_r12_r_23(I1), Plot_Dispersao_r12_r_23(I2),
                      Plot_Dispersao_r12_r_23(I3),Plot_Dispersao_r12_r_23(I4))
    }
    if(entrada == graficos[7])
    {
      y.min = min(dados1[[3]],dados2[[3]],dados3[[3]],dados4[[3]])
      y.max = max(dados1[[4]],dados2[[4]],dados3[[4]],dados4[[4]])
      graficos = list(Plot_Dispersao_s1_r_12.sel(I1,y.min,y.max),
                      Plot_Dispersao_s1_r_12.sel(I2,y.min,y.max),
                      Plot_Dispersao_s1_r_12.sel(I3,y.min,y.max),
                      Plot_Dispersao_s1_r_12.sel(I4,y.min,y.max)
      )
    }
    if(entrada == graficos[8])
    {
      y.min = min(dados1[[3]],dados2[[3]],dados3[[3]],dados4[[3]])
      y.max = max(dados1[[8]],dados2[[8]],dados3[[8]],dados4[[8]])
      graficos = list(Plot_Dispersao_s1_r_23.sel(I1,y.min,y.max),
                      Plot_Dispersao_s1_r_23.sel(I2,y.min,y.max),
                      Plot_Dispersao_s1_r_23.sel(I3,y.min,y.max),
                      Plot_Dispersao_s1_r_23.sel(I4,y.min,y.max)
      )
    }
    if(entrada == graficos[9])
    {
      graficos = list(CORRELACAO_ESPACIAL(I1), CORRELACAO_ESPACIAL(I2),
                      CORRELACAO_ESPACIAL(I3),CORRELACAO_ESPACIAL(I4))
    }
  }
